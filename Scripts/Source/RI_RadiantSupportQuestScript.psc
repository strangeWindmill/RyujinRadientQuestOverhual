...
GlobalVariable Property RI_MBSeniorOperativeAccess Auto Const Mandatory
...
Function MBCompleted()
	;If player is not a senior operative yet, don't bother counting.  
	; Since we're now allowing access to radiants before becoming a senior op, 
	; We could get assigned an in person job before it makes sense.
	Int iPlayerIsSeniorOperative = RI_MBSeniorOperativeAccess.GetValueInt()
	If iPlayerIsSeniorOperative < 1
		return
	EndIf
